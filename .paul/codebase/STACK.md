...
| tailwindcss + @tailwindcss/vite | 4.1.14 | Utility-first CSS (no config file) |
| framer-motion / motion | 12.x | Animations |
| lucide-react | 0.546.0 | Icons |
| recharts | 3.8.1 | Charts di Dashboard/Reports |
...
> **Note:** Neon HTTP driver **tidak support transactions**. Semua operasi multi-step menggunakan sequential inserts — risiko inkonsistensi jika gagal di tengah.

## Storage

| Package | Version | Purpose |
|---------|---------|---------|
| @aws-sdk/client-s3 | 3.1053.0 | Cloudflare R2 upload (S3-compatible API) |

## Unused / Dormant

| Package | Version | Note |
|---------|---------|------|
| @google/genai | 2.4.0 | Terinstall, **belum ada implementasi** di kode |
...
API_PORT=3001          # (optional, dev only) API port
CORS_ORIGIN=           # (optional) Comma-separated allowed origins
```

Template di `.env.example` (perlu dikasih komentar deskripsi per var).
...
- API → Vercel serverless function (`api/index.ts` di root, wraps `src/api/`)
...
```json
{
  ...
  "functions": {
    "api/index.ts": { "maxDuration": 30, "includeFiles": "src/**" }
  }
}
```

**Catatan**: Upload R2 butuh `express.raw()` di handler (recent fix Phase 10).
